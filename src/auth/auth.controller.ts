    ...
    import { Controller, Get, Post, Body, Query, Res, Req, UseGuards, HttpCode, HttpStatus, InternalServerErrorException, Logger, UnauthorizedException, NotFoundException, BadRequestException } from '@nestjs/common';
    ...
    export class AuthController {
      ...
      googleLogin(@Req() req: Request, @Res() res: Response) {
         ...
         try {
           ...
           this.logger.error(`Error during Google login redirect for user ${user.sub}: ${error.message}`, error.stack);
      ...
      @Get('google/callback')
      async googleCallback(@Query('code') code: string, @Query('error') error: string, @Query('state') state: string, @Res() res: Response) {
        this.logger.log(`Received callback from Google. Code: ${code ? '[PRESENT]' : 'N/A'}, Error: ${error || 'N/A'}, State: ${state ? '[PRESENT]' : 'N/A'}`);
        ...
        try {
             ...
             this.logger.error(`Error processing state parameter: ${stateError.message}`, stateError.stack);
             res.status(400).send('Authentication failed: State processing failed.');
             return;
        }

        if (error) {
          this.logger.error(`Error returned from Google OAuth for user ${userId}: ${error}`);
          res.status(401).send(`Authentication failed: ${error}`);
          return;
        }
        if (!code) {
          this.logger.error(`No authorization code received from Google for user ${userId}.`);
        ...
        if (userId === null) {
            ...
            res.status(500).send('Internal server error: User context lost during callback.');
            return;
        }
        // --- End explicit check ---
        ...
        try {
          ...
          this.logger.log(`Successfully obtained and saved tokens for user ${userId}.`);
          res.redirect('/auth/success');

        } catch (err) {
          this.logger.error(`Error exchanging code for tokens for user ${userId}: ${err.message}`, err.stack);
      ...
      async fetchEventsAfterAuth(@Req() req: Request, @Res() res: Response) {
         ...
         try {
             const events = await this.calendarService.listUpcomingEvents(user.sub, 15);
