...
import {
  ...
  UseGuards,
  Req,
} from '@nestjs/common'; // Ensure Get, UseGuards, Req are imported
...
export class ContextController {
  ...
  async getCurrentContextSnapshot(@Req() req: Request): Promise<McpPayload> {
    // Get Req object
    // Extract the user payload attached by JwtAuthGuard
    const user = req.user as JwtPayload;
    this.logger.log(
      `Request received for current context snapshot for user ${user.sub}.`,
    );
