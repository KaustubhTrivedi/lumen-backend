  ...
  async buildContextSnapshot(): Promise<McpPayload> {
    ...
    const [
      ...
      this.calendarService
        .listUpcomingEvents(5) // Fetch up to 5 events
        .catch((error) => {
          // Handle errors during calendar fetching gracefully
          if (error instanceof UnauthorizedException) {
            this.logger.warn(
              `Could not fetch calendar events for context: Not authenticated.`,
            );
          } else {
            this.logger.error(
              `Error fetching calendar events for context: ${error.message}`,
              error.stack,
            );
          }
      ...
      this.tasksService
        .findAll() // Fetch all tasks
        .catch((error) => {
          // Handle errors during task fetching gracefully
          this.logger.error(
            `Error fetching tasks for context: ${error.message}`,
            error.stack,
          );
    ...
    if (
      ...
      this.logger.log(
        `Included ${finalCalendarContext.upcoming_events.length} calendar events.`,
      );
    } else if (
      calendarContextResult.status === 'rejected' ||
      !calendarContextResult.value
    ) {
