        ...
        export class EncryptionService {
          ...
          constructor(private configService: ConfigService) {
            ...
            if (!keyHex || keyHex.length !== 64) {
              this.logger.error('TOKEN_ENCRYPTION_KEY is missing, invalid, or not 64 hex characters (32 bytes) long in .env');
              throw new InternalServerErrorException('Encryption key configuration error.');
            }
          ...
          encrypt(text: string | null | undefined): string | null {
            if (text == null) { // Checks for both null and undefined
              return null;
            }
            try {
          ...
          decrypt(encryptedText: string | null | undefined): string | null {
            ...
            try {
                ...
                if (parts.length !== 3) {
                    this.logger.error('Decryption failed: Invalid encrypted text format (expected 3 parts).');
